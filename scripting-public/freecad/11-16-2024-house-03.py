...
doc = App.newDocument("HouseWithTrees")
...
roof.Ymin = 0
roof.Ymax = 10
roof.Zmin = 10  # Start at the top of the house
roof.Zmax = 16  # Ends above the house
roof.Placement.Base = App.Vector(0, 0, 10)  # Perfectly aligned on the house
...
tree_positions = [-6, -4, -2, 0]  # Positions for tree trunks (left and right side)

# Create trees on the left
for i, x in enumerate(tree_positions):
    trunk = create_cylinder(tree_trunk_radius, tree_trunk_height, x, -3, 0, f"TreeTrunkLeft{i+1}", (0.5, 0.25, 0.0))
    foliage = doc.addObject("Part::Sphere", f"TreeFoliageLeft{i+1}")
    foliage.Radius = tree_foliage_radius
    foliage.Placement.Base = App.Vector(x, -3, tree_trunk_height)  # On top of the trunk
    foliage.ViewObject.ShapeColor = (0.0, 0.5, 0.0)  # Green foliage

# Create trees on the right
for i, x in enumerate(tree_positions):
    trunk = create_cylinder(tree_trunk_radius, tree_trunk_height, x + 26, -3, 0, f"TreeTrunkRight{i+1}", (0.5, 0.25, 0.0))
    foliage = doc.addObject("Part::Sphere", f"TreeFoliageRight{i+1}")
    foliage.Radius = tree_foliage_radius
    foliage.Placement.Base = App.Vector(x + 26, -3, tree_trunk_height)  # On top of the trunk
    foliage.ViewObject.ShapeColor = (0.0, 0.5, 0.0)  # Green foliage

# Recompute the document to apply changes
doc.recompute()

print("House with trees creation complete! Open in FreeCAD to view the model.")
